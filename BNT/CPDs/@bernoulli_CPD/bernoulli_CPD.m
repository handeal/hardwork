function CPD = bernoulli_CPD(bnet, self, varargin)
% BERNOULLI_CPD Make a multinomial conditional prob. distrib. (CPT)
%
% CPD = bernoulli_CPD(bnet, node) creates a random CPT.
%
...
%
% e.g., bernoulli_CPD(bnet, i, 'CPT', T)
% e.g., bernoulli_CPD(bnet, i, 'CPT', 'unif', 'dirichlet_weight', 2, 'dirichlet_type', 'unif')
%
% REFERENCES
...
if nargin==0
  ...
  CPD = init_fields;
  CPD = class(CPD, 'bernoulli_CPD', discrete_CPD(0, []));
  return;
elseif isa(bnet, 'bernoulli_CPD')
...
if senseSize > 1
   fam_sz = [ ns([ps]) senseSize ns(self)] ;
   CPD.sizes = fam_sz;
end
...
CPD = class(CPD, 'bernoulli_CPD', discrete_CPD(~CPD.adjustable, fam_sz));
